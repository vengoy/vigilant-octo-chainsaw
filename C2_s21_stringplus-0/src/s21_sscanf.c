...
void write_unspec_int_to_memory(char **str, int *fail_flag, int *res, token *tok) {
    *fail_flag = 1;
...
void write_chars_to_buff(char **str, const char *chars, char *buff, int16_t width, int start_ind) {
...
void write_float_to_memory(char **str, int *res, token *tok) {
    ...
    if (test) {
        ...
        if (!(sign > 1 || (sign && (tok->width_n <= 1 && tok->width)))) {
            ...
            if (tok->spec == 'f')
                write_chars_to_buff(str, ".0123456789+-", buff, tok->width_n, start_ind);
            else
                write_chars_to_buff(str, ".0123456789eE+-NnaAifIF", buff, tok->width_n, start_ind);
...
void write_hex_or_oct_to_memory(char **str, int *fail_flag, int *res, token *tok, int base) {
    char *ptr = S21_NULL;
    int sign = 1;
    ...
    if (s21_strspn(*str, "0123456789abcdefABCDEF") > 0 || s21_strspn(*str, "xX0123456789abcdefABCDEF") >= 2) {
        unsigned long long int result = s21_strntollu(*str, &ptr, base,
        ...
        if (tok->width != WIDTH_AST) {
            ...
            *res += 1;
            ...
            s21_strntollu(*str, &ptr, base, tok->width ? tok->width_n : s21_strlen(*str));
        }
...
void unsigned_type_converter(token *tok, unsigned long long int result, int sign) {
